...
export default {
  ...
  data() {
    return {
      excelData: [],
      newOwnerName : "IC-Distributor-1",
      newOwnerMSPID : "IC-Distributor-1-MSPID"
    };
  },
  methods: {
    show() {
      this.$modal.show('ocm-sell')
    },
    async submitFile(){

      var response = await PostsService.sellAsset(this.excelData, this.newOwnerName, this.newOwnerMSPID, "OCM");
