...
async function getChallenge(r) {
  ...
  let variable = await PostsService.getChallenge(ecid, "PCB-Assembler");
  ...
  methods: {
    ...
    */
    ...
    async verifyPUFResponse() {
      ...
      let response;
      response = await PostsService.pufVerify(ecidValue,challengeValue,generatedResponse,"PCB-Assembler");
    ...
    async submitFile() {
      ...
      */
      let printString;
      printString = await PostsService.verifyOnLedger(this.excelData, "PCB-Assembler");
