...
async function getChallenge(r) {
  ...
  let variable = await PostsService.getChallenge(ecid, "PCB-Distributor");
  ...
  methods: {
    ...
    */
    ...
    async verifyPUFResponse() {
      ...
      let response;
      response = await PostsService.pufVerify(ecidValue,challengeValue,generatedResponse,"PCB-Distributor");
    ...
    async submitFile() {
      ...
      */
      let printString;
      printString = await PostsService.verifyOnLedger(this.excelData, "PCB-Distributor");
