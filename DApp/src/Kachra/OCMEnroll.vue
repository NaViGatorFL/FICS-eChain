<template>
  <div class="normal-case tracking-normal" >
    ...
    <div class="px-10 py-8" id = "modalReg">
      <div>
        <h1 id="asd">Upload a xlsx file to <b>Enroll</b> assets</h1>
      ...
      <div class="input-group w-50 mx-auto">
        ...
        <div class="input-group-append w-25">
          <button class="btn btn-outline-secondary" type="button" id="inputGroupFileAddon04" v-on:click="submitFile()">Enroll</button>
        </div>
      </div>
      <p id="showStatus"> </p>
    </div>
...
export default {
  ...
  methods: {
    show() {
      this.$modal.show('ocm-enroll')
    },
    async submitFile(){
      var response = await PostsService.createAsset(this.excelData, "OCM");
      console.log(response.data)
      var divContainer = document.getElementById("showStatus");
      divContainer.innerHTML = "<br>" + "<br>" + "Total number of assets submitted for enrollment : " + response.data.Submitted +
          "<br>" + "<br>" + "Number of newly enrolled assets : " + response.data.Enrolled + "<br>" + "<br>" + "Number of enrolled assets that already existed : " + response.data.Rejected;
